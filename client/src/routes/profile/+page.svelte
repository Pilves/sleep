...
<script>
	...
	async function disconnectOura() {
		try {
			...
			ouraConnected = false;
			ouraLastSync = null;
			successMessage = 'Oura Ring disconnected successfully!';
			...
			console.error('Error disconnecting Oura ring:', err);
			error = err.response?.data?.message || 'Failed to disconnect Oura ring. Please try again.';
		} finally {
			loading = false;
		}
	}
				...
				{#if ouraConnected}
					...
					<div class="sync-container">
						...
						>
							...
							{loading ? 'Disconnecting...' : 'Disconnect Oura Ring'}
						</button>
					</div>
				{:else}
					<div class="connected-status">
					...
					>
						{loading ? 'Preparing OAuth...' : 'Connect with Oura OAuth'}
					</button>

					<div class="oauth-info" style="text-align: center; font-size: 0.9rem; color: #666; margin-top: 10px;">
						<p>You'll be redirected to Oura's authorization page to securely connect your account.</p>
					</div>
				{/if}
			</div>
